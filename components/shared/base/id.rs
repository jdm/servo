...
use malloc_size_of::{/*MallocSizeOf, */MallocSizeOfOps, malloc_size_of_is_0};
...
impl Indexable for AnyIndex {
    const DISPLAY_PREFIX: &'static str = "Untyped";
}
...
impl<T> Index<T> {
    ...
    pub fn into_untyped(self) -> Index<AnyIndex> {
        Index(self.0, PhantomData)
    }
}

impl Index<AnyIndex> {
    pub fn into_typed<T: Indexable>(self) -> Index<T> {
        Index(self.0, PhantomData)        
    }
}
...
impl<T> NamespaceIndex<T> {
    pub fn into_untyped(self) -> NamespaceIndex<AnyIndex> {
        NamespaceIndex {
            namespace_id: self.namespace_id,
            index: self.index.into_untyped(),
        }
    }
}

impl NamespaceIndex<AnyIndex> {
    pub fn into_typed<T: Indexable>(self) -> NamespaceIndex<T> {
        NamespaceIndex {
            namespace_id: self.namespace_id,
            index: self.index.into_typed(),
        }
    }
}
...
macro_rules! namespace_id {
    ($id_name:ident, $index_name:ident, $display_prefix:literal) => {
        ...
        //pub type $index_name = Index<Marker_$index_name>;
