...
use crate::webgl_thread::{WebGLExternalImageApi, WebGLExternalImageHandler, WebGLThread, TexturesMap};
...
impl WebGLThreads {
    ...
    pub fn new(
        ...
        let (channel, thread_data) = WebGLThread::start(
            gl_factory,
            webrender_api_sender,
            webvr_compositor.map(|c| Box::new(WebVRRenderWrapper(c)) as Box<_>),
            textures.clone(),
            event_loop_waker,
        );
        ...
        let external =
            ...
            WebGLExternalImageHandler::new(WebGLExternalImages::new(textures));
        (
...
impl WebGLExternalImages {
    ...
    fn new (textures: TexturesMap) -> Self {
        Self {
            textures
        }
    }
...
impl WebGLExternalImageApi for WebGLExternalImages {
    fn lock(&mut self, ctx_id: WebGLContextId) -> (u32, Size2D<i32>) {
        ...
        let entry = textures.get(&ctx_id).expect("no texture entry???");
        (entry.0, entry.1)
    }
