...
impl GLLimitsDetect for GLLimits {
    fn detect(gl: &Gl) -> GLLimits {
        ...
        let max_combined_texture_image_units =
            gl.get_integer(gl::MAX_COMBINED_TEXTURE_IMAGE_UNITS);
        ...
        match gl.try_get_integer(gl::MAX_FRAGMENT_UNIFORM_VECTORS) {
            ...
            None => {
                ...
                let max_vertex_output_components =
                    gl.try_get_integer(gl::MAX_VERTEX_OUTPUT_COMPONENTS).unwrap_or(0);
                let max_fragment_input_components =
                    gl.try_get_integer(gl::MAX_FRAGMENT_INPUT_COMPONENTS).unwrap_or(0);
