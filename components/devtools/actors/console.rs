...
use crate::{ConsoleAPICall, ConsoleMessage, ConsoleMsg, PageErrorMessage};
...
use devtools_traits::{CachedConsoleMessageTypes, ConsoleAPI, DevtoolScriptControlMsg, LogLevel, PageError};
...
impl ConsoleActor {
    ...
    pub(crate) fn handle_page_error(&self, page_error: PageError) {
        self.cached_events.borrow_mut().push(CachedConsoleMessage::PageError(page_error.clone()));
        let msg = PageErrorMessage {
            from: self.name(),
    ...
    pub(crate) fn handle_console_api(&self, console_message: ConsoleMessage) {
        ...
        self.cached_events.borrow_mut().push(CachedConsoleMessage::ConsoleAPI(ConsoleAPI {
            ...
            "getCachedMessages" => {
                ...
                for event in self.cached_events.borrow().iter() {
                    let include = match event {
                        CachedConsoleMessage::PageError(_) if message_types.contains(CachedConsoleMessageTypes::PAGE_ERROR) => true,
                        CachedConsoleMessage::ConsoleAPI(_) if message_types.contains(CachedConsoleMessageTypes::CONSOLE_API) => true,
                        _ => false,
                    };
                    if include {
