...
impl<'a> HTMLIFrameElementMethods for JSRef<'a, HTMLIFrameElement> {
    ...
    fn GetContentWindow(self) -> Option<Temporary<Window>> {
        let containing = self.containing_page_pipeline_id.get();
        if let (Some(containing), Some(subpage)) = (containing, self.subpage_id.get()) {
            ScriptTask::find_frame(containing, Some(subpage), |frame| {
