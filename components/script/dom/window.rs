...
use crate::dom::bindings::codegen::Bindings::AudioContextBinding::AudioContextMethods;
...
pub struct Window {
    ...
    ///
    audio_contexts: DomRefCell<Vec<Dom<AudioContext>>>,
    
    ///
    media_players: DomRefCell<Vec<Dom<HTMLMediaElement>>>,
}

impl Window {
    pub fn register_audio_context(&self, context: &AudioContext) {
        self.audio_contexts.borrow_mut().push(Dom::from_ref(context));
    }
    
    pub fn register_media_player(&self, element: &HTMLMediaElement) {
        self.media_players.borrow_mut().push(Dom::from_ref(element));
    }
...
impl Window {
    ...
    pub fn suspend(&self) {
        ...
        for context in self.audio_contexts.borrow().iter() {
            context.Suspend(InCompartment::entered(&ar));
        }
        
        for element in self.media_players.borrow().iter() {
            element.pause_playback();
        }
    ...
    pub fn resume(&self) {
        ...
        let ar = enter_realm(self);
        for context in self.audio_contexts.borrow().iter() {
            context.upcast::<BaseAudioContext>().Resume(InCompartment::entered(&ar));
        }
...
impl Window {
    ...
    pub fn new(
        ...
        let win = Box::new(Self {
            ...
            player_context,
            event_loop_waker,
            audio_contexts: Default::default(),
            media_players: Default::default(),
        });
