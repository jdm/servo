...
use msg::constellation_msg::{LoadData, PipelineId, SubpageId, ConstellationChan};
...
pub struct Window {
    ...
    browser_context: DOMRefCell<Option<Rc<BrowserContext>>>,
...
impl Window {
    ...
    pub fn browser_context(&self) -> Rc<BrowserContext> {
        self.browser_context.borrow().as_ref().unwrap().clone()
    }
...
impl<'a> WindowMethods for JSRef<'a, Window> {
    ...
    fn Close(self) {
        self.script_chan.send(ScriptMsg::ExitWindow(self.id.clone()));
    }

    fn Document(self) -> Temporary<Document> {
        self.browser_context().active_document()
    }
    ...
    fn GetFrameElement(self) -> Option<Temporary<Element>> {
        self.browser_context().frame_element()
    }
    ...
    fn Parent(self) -> Temporary<Window> {
        ...
        browser_context.frame_element().map_or(self.Window(), |fe| {
            ...
            window.r().browser_context().active_window()
        })
    }
...
impl<'a> WindowHelpers for JSRef<'a, Window> {
    ...
    fn load_url(self, href: DOMString) {
        ...
        if href.as_slice().starts_with("#") {
            self.script_chan.send(ScriptMsg::TriggerFragment(self.id, url));
        } else {
            self.script_chan.send(ScriptMsg::TriggerLoad(self.id, LoadData::new(url)));
        }
    }
